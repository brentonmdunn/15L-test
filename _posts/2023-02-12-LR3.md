...
{: .box-warning}
**Warning:** I found all of the commond line options using https://www.geeksforgeeks.org/grep-command-in-unixlinux/.
...
{: .box-note}
**Note:** Outputs all of the lines that contains "throughout". The `-i` is useful because you don't know if "throughout" is capitalized (such as in the beginnning of the sentence) or not.
...
{: .box-note}
**Note:** Outputs all of the lines that contains "stores". The `-i` is useful because sometimes "stores" is capalized because it is used in a name and sometimes it is lowercase due to it being used as a regular noun reference.
...
```
{: .box-note}
**Note:** Outputs the number of lines that contains "Bond Stores". The `-c` is useful if you want to know in how many lines "Bond Stores" is referenced without needing the exact location or context.
...
```
{: .box-note}
**Note:** Outputs the number of lines that contains "Sydney:". "Sydney:" is written on a new line whenever that character speaks. The `-c` is useful because it can be used to count how many times the character speaks.
...
{: .box-note}
**Note:** Outputs the lines that contains "Father:". "Father:" is written on a new line whenever that character speaks. The `-h` is useful because it can show all of the lines of dialogue for that character.
...
{: .box-note}
**Note:** Outputs the lines that contains "Period". In this context, "Period" is only used to refer to a dynasty. Using `-h`, you can identify the names of all of the dynasties.
...
```
{: .box-note}
**Note:** Outputs file names of all files that contains "History". Could be used if a user wants to read everything that contains history.
...
{: .box-note}
**Note:** Outputs file names of all files that contains "Chapter". Because "Chapter" is written as the first line in this specific book, a user can use `-l` to find all the chapters in the book in this repo.